...
function extractValue(arr, key) {
    return arr.reduce(function(values, nextVal){
        values.push(nextVal[key]);
        return values;
    },[])
...
function vowelCount(str) {
    return str.toLowerCase().split("").reduce(function(accum, nextVal){
        console.log(`accum is ${accum} and nextVal is ${nextVal}`);
        if(accum[nextVal]){
            accum[nextVal] += 1;
        }else{
            if("aeiou".includes(nextVal)){
                accum[nextVal] =1
            }
        }
        return accum;
...
passed to it with each object now including the key 
